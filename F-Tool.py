...
"""
	 handsome list:
		NguyenTuanHung ~ NTH ~ DevHung ~ DvlpNguyenTuanHung ~ HungIT ~ TuanHungIT 
...
class Home:
	...
	def home(self): # don't edit this banner lol
		print(f"""
                        {Color.LG}╔══════════════════════╗
    {Color.LC}╔═╗{Color.LB} ╔╦╗╔═╗╔═╗╦      {Color.LG}║ {Color.LR}Created: {Color.LY}21/12/2022      {Color.LG}║
    {Color.LC}╠╣{Color.LB}{Color.LR}───{Color.LB}║ ║ ║║ ║║      {Color.LG}║ {Color.LR}Updated: {Color.LY}22/4/2022      {Color.LG}║
    {Color.LC}╚{Color.LB}    ╩ ╚═╝╚═╝╩═╝{Color.LG}v1  {Color.LG}║ {Color.LB}Simple but mighty XD {Color.LG}║
                        {Color.LG}╚══════════════════════╝
    {Color.LR}[{Color.LG}>     Made with By NguyenTuanHung    {Color.LG}<{Color.LR}]""")
...
def main():
	...
	try:
		...
		print("\n[+] Please download on GitHub, or git clone: https://github.com/Nghungkali/F-Tool.git\n")
...
{Color.LC}DEV{Color.LR} ~> {Color.LY}Contact/Support dev"""
	dev = f"""{Color.LC}Telegram{Color.LR}: {Color.LY}https://t.me/nghungkali
{Color.LC}Contact{Color.LR}: {Color.LY}092654527"""
